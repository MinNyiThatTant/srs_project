...
class ApplicationApprovalController extends Controller
{
    ...
    public function finalApprove(Request $request, $id)
    {
        ...
        if ($application->application_type === 'new') {
            $message .= ' Student ID: ' . $application->student_id;
        }
    ...
    private function generateStudentId($application)
    {
        ...
        $sequence = Application::where('department', $application->department)
            ->where('status', Application::STATUS_FINAL_APPROVED)
            ->count() + 1;
    ...
    {
        $applications = Application::with(['payments', 'latestPayment'])
            ->orderBy('application_date', 'desc')
            ->paginate(20);
    ...
    /**
     * Reject application (common for all admins)
     */
    ...
    /**
     * Update payment status (for Finance Admin)
     */
