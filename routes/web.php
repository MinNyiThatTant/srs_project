...
// Existing student application routes  
Route::get('old-student-apply', [ApplicationController::class, 'oldStudentForm'])->name('old.student.apply');
Route::post('verify-existing-student', [ApplicationController::class, 'verifyExistingStudent'])->name('verify.existing.student');
Route::post('submit-existing-application', [ApplicationController::class, 'submitExistingApplication'])->name('submit.existing.application');
...
// Debug route for old student application
Route::get('/debug-old-student/{student_id?}', function($student_id = null) {
    try {
        if ($student_id) {
            ...
            $students = \App\Models\Student::where('status', 'active')
                ->limit(10)
                ->get(['student_id', 'name', 'email', 'department']);
